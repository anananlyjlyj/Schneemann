...
gamma_eos = 1.0001#5./3.           # polytropic index of ideal equation of state the run will assume
...
def c_s(r):                 # speed of sound
    return (k_B/mean_molecular_weight * Temp(r))**0.5      
...
Omega = vO_K(r_g)*((1+q) - q*r_g/(r_g**2+z_g**2)**0.5 + (p-(3+q)/2+q) * vAR(r_g)**2)**0.5
...
uv_g = k_B/(mean_molecular_weight*(gamma_eos-1)) * vT(r_g)
