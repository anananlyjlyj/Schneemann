...
def make_IC():
    ...
    r = 1.0
    EPSILON = 1E-16
    Omega = 0.001    # angular velocity, here only in z direction
    ...
    # create a round disk with negligible height
    r2 = (xv_g)**2 + (yv_g)**2
    inDisk = r2 < r**2
    xv_g = xv_g[inDisk];   yv_g = yv_g[inDisk];    zv_g = zv_g[inDisk]
    zv_g *= 0.05    # razor disk   h/r = 0.05
    Ngas = xv_g.size
    
    # set the initial velocity in x/y/ direction (0), in z direction = angular velocity * r
    vx_g=0.*xv_g; vy_g=0.*xv_g; vz_g=Omega*(r2[inDisk])**0.5;
