...
/* Keplerian forces (M=1): useful for orbit, MRI, planetary disk problems */
void GravAccel_KeplerianOrbit()
{
    ...
    /*int i,k; for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {
        ...
        double r2 = (dp[0]-a)*(dp[0]-a)+(dp[1]-b)*(dp[1]-b)+(dp[2]-c)*(dp[2]-c);
        double r = sqrt(r2);
        P[i].GravAccel[0] -= G*(dp[0]-a) / (r2*r);
        P[i].GravAccel[1] -= G*(dp[1]-b) / (r2*r);
        P[i].GravAccel[2] -= G*(dp[2]-c) / (r2*r);
    }*/
    int i; for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {
