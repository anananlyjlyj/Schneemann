...
def make_IC():
    ...
    power = 1;     Sigma_0 = M_disk/(2*np.pi*(r_o-r_i))  # Sigma_0 = Sigma(r = 1 AU) 
    ...
    def Temp(r):
        res = (Q_for_normalize*np.pi*G*Sigma_0/r_o)**2/((k_B*r_o**(-0.5)/mean_molecular_weight)*(G*M/r_o**3)) * r**(-0.5)/100
        res[(r < r_i) | (r > r_o)] = 20
        return res
